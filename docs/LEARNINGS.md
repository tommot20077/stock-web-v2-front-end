...
| 2026-09-04 | `Chart.vue` 的 `theme: props.themeMode === 'mixed' ? 'dark' : 'dark'` —— 三元運算兩邊相同,等於該分支從未生效。同一個物件裡的 `backgroundColor` / `gridColor` 都有正確分支,只有 `theme` 漏改。這種「兩邊一樣的三元」型別檢查與測試都抓不到,只有 lint 或人眼會發現;導入 ESLint 後這類寫法可以靠規則擋 | PR #13;修正抽成 `chartTvTheme.ts` 純函式並補測試 | new |
